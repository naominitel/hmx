...
type crterm = variable CoreAlgebra.term
...
let rec chop pool = function
    ...
    | App (v1, v2) ->
        let (v1, v2) = (chop pool v1), (chop pool v2) in
        let v =
            UnionFind.fresh 
                { (* TEMPORARY invoquer une fonction de c'reation dans Unifier? *)
                    structure = Some (App (Var v1, Var v2));
                    rank = pool.number;
                    mark = Mark.none;
                    kind = Flexible;
                    name = None;
...
let variable_set kind xs =
    StringSet.fold 
        (fun x (vs, xts) ->
             ...
             let v = variable k ?name:n () in
             v :: vs, StringMap.add x ((Var v), undefined_position) xts
        ) xs ([], StringMap.empty)
