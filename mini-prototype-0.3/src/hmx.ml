...
let p = Positions.undefined_position

let rec ty_convert t : Constraint.crterm = match t with
    | TConst v ->
        CoreAlgebra.Var (
            MultiEquation.variable MultiEquation.Constant ~name: (MultiEquation.TName v) ()
        )
    | TVar v ->
        CoreAlgebra.Var (
            MultiEquation.variable MultiEquation.Flexible ~name: (MultiEquation.TName v) ()
        )
    | TApp (t1, t2) ->
        CoreAlgebra.App (ty_convert t1, ty_convert t2)

let rec ast_convert c : Constraint.tconstraint = match c with
    | CBool true -> Constraint.CTrue p
    | CApp (pred, [t1 ; t2]) when pred = is_subtype ->
        Constraint.CEquation (p, ty_convert t1, ty_convert t2)
    | CAnd (c1, c2) -> Constraint.conj [ast_convert c1 ; ast_convert c2]
    | CExists (vars, constr) ->
        Constraint.ex
            (List.map (fun v -> MultiEquation.variable MultiEquation.Flexible ~name:
                              (MultiEquation.TName v) ()) vars)
            (ast_convert constr)
    | CDef (var, sch, constr) ->
        let Forall(v, c, t) = sch in
        let t = ty_convert t in
        let v = 
            List.map
                (fun v -> 
                    MultiEquation.variable MultiEquation.Flexible ~name:
                              (MultiEquation.TName v) ())
                v
        in
        let map = Misc.StringMap.singleton var (t, p) in
        Constraint.CLet (
            [Constraint.Scheme (p, v, [], ast_convert c, map)],
            ast_convert constr
        )
    | CInstance (var, ty) ->
        Constraint.CInstance (p, Constraint.SName var, ty_convert ty)
...
let constr = infer main (TVar (fresh_ty_var ()))

let () =
    Printf.printf "%s\n" @@ show_constr @@ simpl_constr constr ;
    let conv = ast_convert @@ simpl_constr constr in
    let solved = Solver.solve conv in
    let rec dump = function
        | Solver.EEmpty -> ()
        | Solver.EEnvFrame (n, v, t) ->
            Printf.printf "miaou " ; dump n
    in dump solved ; print_newline () ;
    ()
